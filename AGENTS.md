...
- BEFORE editing God Nodes or bridge nodes (`MisgaDatabaseHelper`, `SmsRepository`, `ConversationsViewModel`, `FilterRule`, `ChatViewModel`, `FilterAction`): consult `graphify-out/graph.json` + `GRAPH_REPORT.md` (query the map, check community + blast radius). Never touch them silently.
